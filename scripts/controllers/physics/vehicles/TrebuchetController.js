...
define(["controllers/physics/actors/ActorsController",
	...
	var TrebuchetController = function( name, 
										canvas, 
										context, 
		...
		var trebuchet_chassis_body; //b2Body;
		var trebuchet_arm_body; //b2Body;
		...
		var following_trebuchet = false;

		var actorsController = new ActorsController(actors, bodies, world, SCALE);

		var buildCannonball = function() {
			//console.log("TrebuchetController / buildCannonball / x: " + vehicleStartPosX);

			var cannonball = new b2BodyDef();
			cannonball.position.Set(vehicleStartPosX/SCALE, vehicleStartPosY/SCALE);
		...
		var buildWheelMotors = function() {
			...
			front_wheel_joint.Initialize(trebuchet_chassis_body, front_wheel_body,new b2Vec2(0,0));
			...
			rear_wheel_joint.Initialize(trebuchet_chassis_body, rear_wheel_body, new b2Vec2(0,0));
		...
		var buildTrebuchetMotor = function() {
			var arm_joint = new b2RevoluteJointDef();
			arm_joint.enableMotor = true;
			arm_joint.enableLimit = true;

			arm_joint.Initialize(trebuchet_chassis_body, trebuchet_arm_body, new b2Vec2(0,0) );
		...
		var buildTrebuchetArm = function() {
			var trebuchet_arm = new b2BodyDef();
			trebuchet_arm.allowSleep = false;
			trebuchet_arm.position.Set(210/SCALE,110/SCALE);
			trebuchet_arm.type = b2Body.b2_dynamicBody;
			...
			trebuchet_arm_body = world.CreateBody(trebuchet_arm);
			trebuchet_arm_body.CreateFixture(arm_part_fixture);
			trebuchet_arm_body.CreateFixture(stopper_fixture);
		}

		var buildTrebuchetBody = function() {
			
			var trebuchet_body = new b2BodyDef();
			trebuchet_body.position.Set(vehicleStartPosX/vehicleScaleX,vehicleStartPosY/vehicleScaleY);
			trebuchet_body.type = b2Body.b2_dynamicBody;
			...
			trebuchet_chassis_body = world.CreateBody(trebuchet_body);
			trebuchet_chassis_body.CreateFixture(chassis_fixture);
			trebuchet_chassis_body.CreateFixture(fixed_arm_fixture);
			
		}

		var setMaxMotorTorque = function( maxTorque ) {
			arm_revolute_joint.SetMaxMotorTorque(maxTorque);
			following_trebuchet = false;
		}; 
		...
		var getCameraFocus = function() {
			...
			if (following_trebuchet) {
				pos_x = trebuchet_chassis_body.GetWorldCenter().x * SCALE;
				pos_y = trebuchet_chassis_body.GetWorldCenter().y * SCALE;
			} else {
